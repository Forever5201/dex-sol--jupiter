/**
 * Orca Whirlpool 交易构建器
 *
 * 支持本地构建 Orca Whirlpool 的 Swap 指令，无需调用外部API
 * 参考: https://github.com/orca-so/whirlpools-sdk
 */

import {
  PublicKey,
  TransactionInstruction,
  ComputeBudgetProgram,
  SYSVAR_RENT_PUBKEY,
  SYSVAR_CLOCK_PUBKEY,
  AccountMeta
} from '@solana/web3.js';
import BN from 'bn.js';
import { InstructionBuilder } from './instruction-builder';
import { PriceData } from '../parsers/raydium';
import { createLogger } from '@solana-arb-bot/core';

const logger = createLogger('OrcaWhirlpoolBuilder');

// Orca Whirlpools Program ID
const ORCA_WHIRLPOOLS_PROGRAM_ID = new PublicKey(
  'whirLbMiicVdio4qvUfM5KAg6Ct8VwpYzGff3uctyCc'
);

// Token Program ID
const TOKEN_PROGRAM_ID = new PublicKey(
  'TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA'
);

// Associated Token Program ID
const ASSOCIATED_TOKEN_PROGRAM_ID = new PublicKey(
  'ATokenGPvbdGVxr1b2hvZbsiqW5xWH25efTNsLJA8knL'
);

// Memo Program ID
const MEMO_PROGRAM_ID = new PublicKey(
  'MemoSq4gqABAXKb96qnH8TysNcWxMyWCqXgDLGmfcHr'
);

export class OrcaWhirlpoolBuilder implements InstructionBuilder {
  /**
   * 构建Whirlpool Swap交易指令
   */
  async buildSwap(
    pool: PriceData,
    inputAmount: BN,
    minOutput: BN,
    userWallet: PublicKey,
    priorityFeeMicroLamports: number = 20000
  ): Promise<TransactionInstruction> {
    try {
      logger.debug(`Building swap for Whirlpool: ${pool.poolAddress}`);

      // 获取池子的链上数据
      const poolData = await this.getPoolData(new PublicKey(pool.poolAddress));

      // 计算Tick Array地址（基于当前tick）
      const tickArrayAddresses = await this.getTickArrayAddresses(
        new PublicKey(pool.poolAddress),
        pool.clmmData?.tickCurrent || 0,
        pool.clmmData?.tickSpacing || 1
      );

      // 获取用户的代币账户
      const inputTokenAccount = await this.getAssociatedTokenAccount(
        new PublicKey(pool.baseReserve > 0 ? pool.baseReserve.toString() : pool.quoteReserve.toString()),
        userWallet
      );
      const outputTokenAccount = await this.getAssociatedTokenAccount(
        new PublicKey(pool.baseReserve > 0 ? pool.quoteReserve.toString() : pool.baseReserve.toString()),
        userWallet
      );

      // 确定交易方向 (aToB)
      // 根据池子的代币顺序和输入输出来确定
      const tokenMintA = poolData.tokenMintA;
      const tokenMintB = poolData.tokenMintB;
      const inputMint = new PublicKey(pool.baseReserve > 0 ? pool.baseReserve.toString() : pool.quoteReserve.toString());

      // 如果输入代币是tokenMintA，则aToB为true
      const aToB = tokenMintA.equals(inputMint);

      // 构建账户数组
      const keys = this.buildAccountMetas(
        poolData,
        inputTokenAccount,
        outputTokenAccount,
        tickArrayAddresses,
        userWallet
      );

      // 构建指令数据
      const data = this.encodeSwapV2Data(
        inputAmount,
        minOutput, // 使用最小输出金额作为otherAmountThreshold
        new BN(0), // sqrtPriceLimit: 0 表示无限制
        true,      // amountSpecifiedIsInput: true 表示输入金额固定
        aToB       // 交易方向
      );

      // 创建主要的swap指令
      const swapInstruction = new TransactionInstruction({
        programId: ORCA_WHIRLPOOLS_PROGRAM_ID,
        keys,
        data
      });

      // 设置计算单元限制和价格
      const computeUnitLimitIx = ComputeBudgetProgram.setComputeUnitLimit({
        units: this.getComputeUnitLimit(pool),
      });

      const computeUnitPriceIx = ComputeBudgetProgram.setComputeUnitPrice({
        microLamports: priorityFeeMicroLamports,
      });

      logger.debug(
        `Successfully built Whirlpool swap instruction with ${priorityFeeMicroLamports} micro-lamports/CU`
      );

      // 返回包含优先费指令的完整指令数组
      // 合并所有指令到一个单独的TransactionInstruction（通过创建一个复合指令）
      // 实际上，我们需要返回一个数组，但修改主类以处理指令数组
      return swapInstruction; // For now, return single instruction
    } catch (error) {
      logger.error(`Failed to build Whirlpool swap: ${error}`);
      throw error;
    }
  }

  /**
   * 检查是否支持该池子
   */
  canBuild(pool: PriceData): boolean {
    return pool.dex.includes('Whirlpool') || pool.poolType === 'CLMM';
  }

  /**
   * 获取构建交易所需的额外账户
   */
  getRequiredAccounts(pool: PriceData): PublicKey[] {
    const accounts: PublicKey[] = [];

    // 需要获取tick array账户，基于当前tick和tick spacing
    // 这里返回池子地址本身，实际tick arrays需要根据池子数据动态计算
    accounts.push(new PublicKey(pool.poolAddress));

    return accounts;
  }

  /**
   * 获取推荐的计算单元限制
   * @param pool 价格数据
   * @returns 计算单元数
   */
  getComputeUnitLimit(pool: PriceData): number {
    // Whirlpool typically requires more compute units due to cross-tick calculations
    return 350_000; // 350k CUs for Whirlpool swaps
  }
  
  /**
   * 检查是否支持该池子
   */
  canBuild(pool: PriceData): boolean {
    return pool.dex.includes('Whirlpool') || pool.poolType === 'CLMM';
  }
  
  /**
   * 获取构建交易所需的额外账户
   */
  getRequiredAccounts(pool: PriceData): PublicKey[] {
    const accounts: PublicKey[] = [];
    
    // 需要获取tick array账户，基于当前tick和tick spacing
    // 这里返回池子地址本身，实际tick arrays需要根据池子数据动态计算
    accounts.push(new PublicKey(pool.poolAddress));
    
    return accounts;
  }
  
  /**
   * 获取池子数据（模拟从链上获取）
   */
  private async getPoolData(poolAddress: PublicKey): Promise<any> {
    // 这里应该从链上获取池子数据
    // 模拟返回一个基础对象，实际实现需要根据链上数据构建
    return {
      whirlpoolsConfig: poolAddress, // 实际的config地址
      whirlpoolBump: new BN(0),
      tickSpacing: new BN(64), // 默认tick spacing
      feeTierKey: poolAddress, // 实际的fee tier地址
      tokenMintA: new PublicKey('So11111111111111111111111111111111111111112'), // 示例token
      tokenMintB: new PublicKey('EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v'), // 示例token
      tokenVaultA: poolAddress, // 实际vault地址
      tokenVaultB: poolAddress, // 实际vault地址
      feeAccount: poolAddress, // 实际fee account
      authority: poolAddress,
      rewardLastUpdatedTimestamp: new BN(0),
      feeGrowthGlobalA: new BN(0),
      feeGrowthGlobalB: new BN(0),
      treasuryFeeGrowthA: new BN(0),
      treasuryFeeGrowthB: new BN(0),
      positionBump: new BN(0),
      tickCurrentIndex: 0,
      sqrtPrice: new BN(0),
      liquidity: new BN(0),
      protocolFeeOwedA: new BN(0),
      protocolFeeOwedB: new BN(0)
    };
  }
  
  /**
   * 获取Tick Array地址（基于当前tick和tick spacing）
   */
  private async getTickArrayAddresses(
    poolAddress: PublicKey,
    currentTick: number,
    tickSpacing: number
  ): Promise<PublicKey[]> {
    const addresses: PublicKey[] = [];
    
    // 计算tick array size (一般为tickSpacing * 64)
    const tickArraySize = tickSpacing * 64;
    
    // 计算当前tick array的起始index
    const startTickIndex = Math.floor(currentTick / tickArraySize) * tickArraySize;
    
    // 生成连续的tick array地址（一般需要3个）
    for (let i = 0; i < 3; i++) {
      const tickArrayStartIndex = startTickIndex + (i * tickArraySize);
      const [tickArrayAddress] = await PublicKey.findProgramAddress(
        [
          Buffer.from('tick_array'),
          poolAddress.toBuffer(),
          this.i32ToBytes(tickArrayStartIndex)
        ],
        ORCA_WHIRLPOOLS_PROGRAM_ID
      );
      
      addresses.push(tickArrayAddress);
    }
    
    return addresses;
  }
  
  /**
   * 将i32转换为字节
   */
  private i32ToBytes(value: number): Buffer {
    const buffer = Buffer.alloc(4);
    buffer.writeInt32LE(value, 0);
    return buffer;
  }
  
  /**
   * 将u128转换为字节
   */
  private u128ToBytes(value: BN): Buffer {
    const buffer = Buffer.alloc(16);
    const hex = value.toString(16).padStart(32, '0');
    
    for (let i = 0; i < 16; i++) {
      const byte = hex.slice((15 - i) * 2, (15 - i) * 2 + 2);
      buffer.writeUInt8(parseInt(byte, 16), i);
    }
    
    return buffer;
  }
  
  /**
   * 构建账户元数据
   */
  private buildAccountMetas(
    poolData: any,
    inputTokenAccount: PublicKey,
    outputTokenAccount: PublicKey,
    tickArrayAddresses: PublicKey[],
    userWallet: PublicKey
  ): AccountMeta[] {
    const keys: AccountMeta[] = [];
    
    // 0. Token Program A
    keys.push({
      pubkey: TOKEN_PROGRAM_ID,
      isSigner: false,
      isWritable: false
    });
    
    // 1. Token Program B
    keys.push({
      pubkey: TOKEN_PROGRAM_ID,
      isSigner: false,
      isWritable: false
    });
    
    // 2. Memo Program (optional)
    keys.push({
      pubkey: MEMO_PROGRAM_ID,
      isSigner: false,
      isWritable: false
    });
    
    // 3. Token Authority (user wallet)
    keys.push({
      pubkey: userWallet,
      isSigner: true,
      isWritable: false
    });
    
    // 4. Whirlpool (pool state)
    keys.push({
      pubkey: new PublicKey(poolData.whirlpoolsConfig),
      isSigner: false,
      isWritable: true
    });
    
    // 5. Token Mint A
    keys.push({
      pubkey: poolData.tokenMintA,
      isSigner: false,
      isWritable: false
    });
    
    // 6. Token Mint B
    keys.push({
      pubkey: poolData.tokenMintB,
      isSigner: false,
      isWritable: false
    });
    
    // 7. Input Token Owner Account
    keys.push({
      pubkey: inputTokenAccount,
      isSigner: false,
      isWritable: true
    });
    
    // 8. Token Vault A
    keys.push({
      pubkey: poolData.tokenVaultA,
      isSigner: false,
      isWritable: true
    });
    
    // 9. Output Token Owner Account
    keys.push({
      pubkey: outputTokenAccount,
      isSigner: false,
      isWritable: true
    });
    
    // 10. Token Vault B
    keys.push({
      pubkey: poolData.tokenVaultB,
      isSigner: false,
      isWritable: true
    });
    
    // 11. Tick Array 0
    keys.push({
      pubkey: tickArrayAddresses[0],
      isSigner: false,
      isWritable: true
    });
    
    // 12. Tick Array 1
    keys.push({
      pubkey: tickArrayAddresses[1],
      isSigner: false,
      isWritable: true
    });
    
    // 13. Tick Array 2
    keys.push({
      pubkey: tickArrayAddresses[2],
      isSigner: false,
      isWritable: true
    });
    
    // 14. Oracle
    keys.push({
      pubkey: SYSVAR_CLOCK_PUBKEY,
      isSigner: false,
      isWritable: false
    });
    
    return keys;
  }
  
  /**
   * 编码Swap V2指令数据
   */
  private encodeSwapV2Data(
    amount: BN,
    otherAmountThreshold: BN,
    sqrtPriceLimit: BN,
    amountSpecifiedIsInput: boolean,
    aToB: boolean
  ): Buffer {
    // Anchor Swap V2 指令格式:
    // - 8 bytes: discriminator
    // - 8 bytes: amount
    // - 8 bytes: otherAmountThreshold
    // - 16 bytes: sqrtPriceLimit
    // - 1 byte: amountSpecifiedIsInput
    // - 1 byte: aToB
    
    const discriminator = Buffer.from([
      0x0a, 0x8b, 0x5e, 0xcd, 0xb7, 0x51, 0x89, 0x83  // swapV2 discriminator
    ]);
    
    const buffer = Buffer.alloc(42); // 8 + 8 + 8 + 16 + 1 + 1
    let offset = 0;
    
    // 1. discriminator
    discriminator.copy(buffer, offset);
    offset += 8;
    
    // 2. amount (u64)
    buffer.writeBigUInt64LE(BigInt(amount.toString()), offset);
    offset += 8;
    
    // 3. otherAmountThreshold (u64)
    buffer.writeBigUInt64LE(BigInt(otherAmountThreshold.toString()), offset);
    offset += 8;
    
    // 4. sqrtPriceLimit (u128)
    const sqrtPriceLimitBytes = this.u128ToBytes(sqrtPriceLimit);
    sqrtPriceLimitBytes.copy(buffer, offset);
    offset += 16;
    
    // 5. amountSpecifiedIsInput (bool)
    buffer.writeUInt8(amountSpecifiedIsInput ? 1 : 0, offset);
    offset += 1;
    
    // 6. aToB (bool)
    buffer.writeUInt8(aToB ? 1 : 0, offset);
    
    return buffer;
  }
  
  /**
   * 获取关联代币账户地址
   */
  private async getAssociatedTokenAccount(
    mint: PublicKey,
    owner: PublicKey
  ): Promise<PublicKey> {
    const [ata] = PublicKey.findProgramAddressSync(
      [
        owner.toBuffer(),
        TOKEN_PROGRAM_ID.toBuffer(),
        mint.toBuffer()
      ],
      ASSOCIATED_TOKEN_PROGRAM_ID
    );
    
    return ata;
  }
}

export default OrcaWhirlpoolBuilder;